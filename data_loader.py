...
class Data_Loader:
    def __init__(self,options):
        ...
        itemfreq = {}
        for xs in tqdm(x_list):
            for x in xs:
                itemfreq[self.item_dict[x]] = itemfreq.setdefault(self.item_dict[x],0) + 1
        ...
        self.maxtarget = len(self.item_dict)
...
class Data_Loader_Inference:
    def __init__(self,options):
        ...
        """
        index_data_file = options['dir_name_index']
        index_data_file_re = options['dir_name_index_re']
        ...
        """
        print("Map the raw data to their respective item indices")
        self.item_dict = self.re_dict(self.item_dict, x_list)

        """Save the "item index dictionary" to the "dir_name_index" path. (i.e., index_data_file)"""
        f = open(index_data_file_re,'w')
        f.write(str(self.item_dict))
        f.close()
        print("The index has been written to {}".format(index_data_file_re))
    ...
    def re_dict(self, dict_pretrain, source):
        max_value = max(dict_pretrain.values())+1
        i = 0
        for lines in tqdm(source):
            for x in lines:
                try:
                    ...
                    dict_pretrain[x] = max_value + i
                    i+=1
        return dict_pretrain
...
class Data_Loader_Sup:
    def __init__(self,options):
        ...
        Load the "item index dictionary."
        """
        index_data_file = options['dir_name_index']
        index_data_file_re = options['dir_name_index_re']
        self.item_dict = self.read_dict(index_data_file) #Load the "item index dictionary."
        ...
        """
        print("Map the raw data to their respective item indices")
        self.item_dict = self.re_dict(self.item_dict, source)
        self.item_dict = self.re_dict(self.item_dict, target)
        ...
        for line in trange(lens):
            ...
            for t in target_line: #split the targets independently
                unit = np.append(source_line, np.array(self.item_dict[t]))
