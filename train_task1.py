...
def main():
    ...
    parser.add_argument('--lr', type = float, default=0.01,
                        help='Learning rate')
    parser.add_argument('--datapath', type=str, default='./Tmall/task1_click.csv',
                        help='data path')
    parser.add_argument('--datapath_index', type=str, default='Data/Tmall/index_task1.csv',
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_sam in enumerate(getBatch(train_set, batch_size)):
            ...
            neg_target = np.random.choice(len(items), targets.shape[0]*args.n_neg,p=dl.prob)
            neg_target = torch.LongTensor(neg_target).to(args.device)
            
            target_emb_positive = model.embeding(targets)
            ...
            pos_logits = pos_logits.unsqueeze(1).repeat(1,99,1).squeeze(2)
            pos_logits = pos_logits.squeeze(0)
            neg_logits = torch.sum((outputs_dup*target_emb_negative),dim=-1)

            # logits = torch.cat((pos_logits,neg_logits),dim=0)
            # lab = torch.zeros(logits.shape[0]).to(args.gpu_num)
            loss = -(pos_logits-neg_logits).sigmoid().log().sum()
        ...
        if epoch == 0:
            best_acc = sum(result_[0]) / float(len(result_[0]))
            count = 0
            list__ = [[] for i in range(6)]
            print('-----testing in best validation-----')
            model_test_(model_para, test_set, model, epoch, args, list__,smax,len(items),dl.prob)
        else:
            if best_acc < sum(result_[0]) / float(len(result_[0])):
                best_acc = sum(result_[0]) / float(len(result_[0]))
                count = 0
                list__ = [[] for i in range(6)]
                print('-----testing in best validation-----')
